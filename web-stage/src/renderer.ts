...
function colorFromPacked(value: number): string {
  const r = value & 0xff;
...
function readCommand(view: DataView, offset: number) {
  return {
...
export function createRenderer(canvas: HTMLCanvasElement) {
  ...
  function resize() {
    ...
    if (!parent) {
      throw new Error("Stage canvas is missing a parent element.");
    }
  ...
  function render(frame: RenderFrame) {
    ...
    const commandView = new DataView(
      frame.memory.buffer,
      frame.commandsPtr,
      frame.commandsLen * COMMAND_STRIDE,
    );
